...
sequenceDiagram
    ...
    loop 游标分片
        ...
        G->>D: 失败时 WAIT→DISPATCH_FAILED；Outbox 写 PUBLISHED/DEAD
        G-->>P: 分片进度帧
    end
```
...
2. 正常路径立即发布，降低端到端延迟。
3. 发布成功后执行记录保持 WAIT、Outbox 改为 PUBLISHED；确定投递失败则改为 DISPATCH_FAILED/DEAD。
...
调用方重试时复用 `request_id`。数据库唯一键 `(request_id, case_id)` 处理并发重试，插入冲突后回读已有执行记录和 Outbox。消息表已是 PUBLISHED、PROCESSING 或 DEAD 时直接返回既有下发结果，不重复投递。

下发回写 DISPATCH_FAILED 使用 `WHERE execution_status = 'WAIT'`。如果执行机处理很快，已经通过 Python Web 把状态推进到 RUNNING 或 SUCCESS，迟到的 DISPATCH_FAILED 回写不会造成状态回退。更新没有命中时会再次查询：记录存在表示状态已被合法推进，记录不存在才是错误。

## 7. 取消与优雅停机

扫描、工作协程、RabbitMQ 借用与确认等待都响应请求上下文。MQ 发布完成后，状态回写使用 `context.WithoutCancel` 派生的 5 秒补偿上下文，避免客户端断开导致 Outbox 永久停在 PENDING。
